...
| 7 Android/loja | **COMPLETA (código)** | e6925f0 | store-safe por flag + E2E verde + chunk AdultoPage removido por DCE + LiveTV bloqueia canal adulto + buildTypes avaliados (flavors desnecessários) | nada de código; só assinar release (Fase 0, infra) |
...
- [x] Separar variant Android nativa/flavor: **dispensável** — chunk adulto já removido por DCE (build store sem `AdultoPage-*.js`). Flavor só seria necessário se a loja exigisse separação por buildType Gradle de outros artefatos.
- [x] **buildTypes avaliados — adequados, sem necessidade de product flavors.** `android/app/build.gradle` já tem `release` endurecido (debuggable false, minifyEnabled, shrinkResources, proguard, signingConfig + validação fail-closed de keystore/senha em `afterEvaluate`) e `debug` (debuggable, sem minify). Os "targets" tv/web/legacy vivem 100% no bundle web (`VITE_APP_TARGET`), não no código nativo: o Capacitor copia um único `dist/` para o shell Android, que é agnóstico de target. Logo, criar product flavors Gradle (tv/web/legacy) adicionaria complexidade sem benefício e contra a regra "não quebrar". Decisão: manter só debug/release.
...
- **Fase 7 (buildTypes)** — Avaliado `android/app/build.gradle`: buildTypes `debug`/`release` já formais e endurecidos (release: minify+shrink+proguard+assinatura+validação fail-closed). Targets tv/web/legacy são bundle Vite (`VITE_APP_TARGET`), não código nativo — Capacitor copia `dist/` único num shell agnóstico. Product flavors Gradle seriam complexidade sem benefício → **decisão: não criar**. Fase 7 fechada no que toca a código; resta só assinatura release (Fase 0, infra).
- **Próximo (tudo restante depende de VOCÊ):** (a) keystore release `android/redx-release.jks` + `EXPECTED_SIGNATURE` → fecha Fase 0; (b) `supabase login` + senha DB → `db pull` (migrations/RLS) + `functions download` → fecha Fase 1; (c) opcional: elevar cobertura de testes por área (Fase 5). Sem essas credenciais, não há mais item de código executável.
